...
package org.nlpcraft.nlp.enrichers.date.tools

import java.text.{DateFormat, SimpleDateFormat}
import java.util.{Date, Locale, Calendar ⇒ C}

import org.nlpcraft._
import org.nlpcraft.nlp.enrichers.date.NCDateConstants._
...
object NCDateGenerator {
    ...
    private[date] val FMT_DATES =
        ...
        mkFormatters(YEAR_MONTH_DAY3, dupD = true, dupM = true)

    private[date] val FMT_DAYS_YEAR =
        mkFormatters(MONTH_DAY1, dupD = true, dupM = false) ++
        mkFormatters(MONTH_DAY2, dupD = true, dupM = false) ++
        mkFormatters(MONTH_DAY3, dupD = false, dupM = false)

    private[date] val FMT_MONTHS = MONTH_YEAR.map(new SimpleDateFormat(_))
    private[date] val FMT_YEARS = YEAR.map(new SimpleDateFormat(_))

    private val FMT_DAY = new SimpleDateFormat("d")

    private[date] val FMT_DATES_DIGITS = mkDigitFormatters(YEAR_MONTH_DAY1 ++ YEAR_MONTH_DAY2)
    private[date] val FMT_DAYS_YEAR_DIGITS = mkDigitFormatters(MONTH_DAY1 ++ MONTH_DAY2)
    ...
    private val YEARS_SEQ = for (i ← 2010 to 2024) yield i
    ...
    private def mkMonths(y: Int, m: String) = {
        ...
        FMT_MONTHS.map(_.format(d).toLowerCase)
    }
    ...
    private[date] def months(df: LHM_SS): Unit = {
        for (y ← YEARS_SEQ; m ← MMMM_MONTH_SEQ)
            mkMonths(y, m).foreach(s ⇒ df += s"$s" → s"${y}y, ${MMMM_MONTH_MAP(m)}m")
    ...
    private[date] def days(df: LHM_SS): Unit = {
        ...
        for (d ← days)
            format(d.date, FMT_DAYS_YEAR).foreach(s ⇒ df += s → s"${d.m}m, ${d.d}d")

        for (d ← days)
            format(d.date, FMT_DAYS_YEAR_DIGITS).foreach(s ⇒ df += s → s"${d.m}m, ${d.d}d")
    }

    private[date] def dates(df: LHM_SS): Unit = {
        val days = mkDays(YEARS_SEQ.head, YEARS_SEQ.last + 1)

        // Dates with two digits day of months will be duplicated with these formatters set,
        // but they will be skipped because added to the map.
        for (d ← days)
            format(d.date, FMT_DATES).foreach(s ⇒ df += s → s"${d.y}y, ${d.m}m, ${d.d}d")
...
object NCDateGeneratorRunner extends App {
    private val FULL_FILE = mkPath("date/full.txt")
