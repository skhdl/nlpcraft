...
package org.nlpcraft.nlp.enrichers.stopword

import org.nlpcraft.nlp.enrichers.{NCNlpEnricher, NCNlpEnricherUtils}
import org.nlpcraft.nlp.pos.NCPennTreebank
import org.nlpcraft.nlp.{NCCoreNlp, NCNlpSentence, NCNlpSentenceNote, NCNlpSentenceToken}
 ...
 */
object NCStopWordEnricher extends NCNlpEnricher("STOP enricher") {
    // Condition types.
    ...
    private final val PERCENTS = NCCoreNlp.stemmatizeSeq(Seq(
        "%",
        "pct",
        "pc",
        "percentage",
    ...
    private def readStopWords(lines: Seq[String]): Map[Boolean, StopWordHolder] = {
        ...
        class Condition[T] {
            val any: HSet[T] = HSet.empty[T]
            val includes: HMap[String, HSet[T]] = HMap.empty[String, HSet[T]]
            val excludes: HMap[String, HSet[T]] = HMap.empty[String, HSet[T]]

            def addCondition(cond: T, poses: Map[String, Boolean]): Any =
                if (poses.isEmpty)
                    any += cond
                else {
                    def add(m: HMap[String, HSet[T]], incl: Boolean): Unit =
                        poses.filter { case (_, isIncl) ⇒ isIncl == incl }.keys.foreach(pos ⇒
                            m.get(pos) match {
                                case Some(set) ⇒ set += cond
                                case _ ⇒
                                    val set = HSet.empty[T]
            ...
            if (idxWild < 0) {
                val (word, form) =
                    if (isCase)
                        ...
                        if (!hasPoses) (NCCoreNlp.stemmatize(s), STEM) else (NCCoreNlp.lemmatize(s), LEM)
                    }
        ...
        def toImmutable[T](m: HMap[String, HSet[T]]): Map[String, Set[T]] = m.map(p ⇒ p._1 → p._2.toSet).toMap
     ...
     * @param tok Token.
     */
    private def mark(tok: NCNlpSentenceToken): Unit = tok.getNlpNote += "stopWord" → true
    ...
    private def exists(
        ...
        cache: HMap[Seq[NCNlpSentenceToken], Boolean], 
        get: Seq[NCNlpSentenceToken] ⇒ Boolean
    ): Boolean =
    ...
    private def processBrackets(ns: NCNlpSentence): Unit = {
        ...
        def process(): Unit =
            ns.filter(!_.isQuoted).find(NCNlpEnricherUtils.isBR) match {
                case Some(t) ⇒
                    // Invalid sentence if first bracket is right.
                    if (NCNlpEnricherUtils.isRBR(t))
                    ...
                    for (tok ← copy) {
                        if (NCNlpEnricherUtils.isLBR(tok) && !tok.isQuoted) {
                            inBrackets = true
                        ...
                        if (NCNlpEnricherUtils.isRBR(tok) && !tok.isQuoted)
                            if (!inBrackets)
                                ...
                                if (!inBrackets) {
                                    val origText = NCNlpEnricherUtils.mkSumString(buf, (t: NCNlpSentenceToken) ⇒ t.origText)
                                    ...
                                    val note = NCNlpSentenceNote(
                                        ...
                                        "lemma" → NCNlpEnricherUtils.mkSumString(buf, (t: NCNlpSentenceToken) ⇒ t.lemma),
                                        "origText" → origText,
                                        "normText" → NCNlpEnricherUtils.mkSumString(buf, (t: NCNlpSentenceToken) ⇒ t.normText),
                                        "stem" → NCNlpEnricherUtils.mkSumString(buf, (t: NCNlpSentenceToken) ⇒ t.stem),
    ...
    override def enrich(ns: NCNlpSentence) {
        ...
        val cacheSw = HMap.empty[Seq[NCNlpSentenceToken], Boolean]
        val cacheEx = HMap.empty[Seq[NCNlpSentenceToken], Boolean]
        ...
        for (tup ← origToks; key = tup._2 if !foundKeys.contains(key) && !isException(tup._1))
            foundKeys.find(key.startsWith) match {
                case Some(s) ⇒ if (NOUN_WORDS.contains(key.substring(s.length).trim)) tup._1.foreach(mark)
                case None ⇒ ()
            }
